...
<!-- ![Sơ đồ kiến trúc AWS của SpendWise](/assets/aws-architecture.jpg) -->
...
| **Amazon Route 53 & ACM** | Tên miền API và TLS khi dùng custom domain cho backend. |
...
- **Tuần 1 - Foundation**
  - Chốt yêu cầu chức năng và NFR.
  - Khởi tạo Terraform backend/state strategy.
  - Dựng VPC, SG, ALB, ECR, ECS cơ bản.

- **Tuần 2 - Identity & App Deployment**
  - Tích hợp Cognito (đăng ký/đăng nhập).
  - Đẩy backend image lên ECR và rollout ECS.
  - Kết nối frontend Amplify với API endpoint thực tế.

- **Tuần 3 - Data & Hardening**
  - Bật RDS và migration dữ liệu.
  - Kiểm tra private networking, security hardening.
  - Thiết lập HTTPS/custom domain (nếu áp dụng).

- **Tuần 4 - Stabilization & Handover**
  - Kiểm thử hiệu năng cơ bản.
  - Thiết lập giám sát, cảnh báo, runbook vận hành.
  - Đánh giá chi phí thực tế và kế hoạch tối ưu tháng tiếp theo.
...
| Amazon Route 53 | Hosted zone + DNS query | $0 - $5 |
...
- Có thể tạm tắt custom domain (Route 53 + ACM), sử dụng endpoint mặc định để giảm chi phí cố định.
