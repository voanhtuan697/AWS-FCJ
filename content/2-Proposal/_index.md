...
Spendwise is a full-stack personal expense management web application that helps users track, analyze, and control personal finance in a practical and visual way.
...
To address these issues, Spendwise adopts an AWS cloud architecture:
...
| **Amazon Route 53 & ACM** | Custom domain and TLS certificate management for API endpoint. |
...
- **Week 1 - Foundation**
  ...
  - Initialize Terraform workflow and state strategy.
  - Stand up baseline VPC, SG, ALB, ECR, ECS.

- **Week 2 - Identity & App Deployment**
  - Integrate Cognito authentication flows.
  - Push backend image to ECR and deploy ECS service.
  - Connect Amplify frontend to live backend endpoint.

- **Week 3 - Data & Hardening**
  - Enable RDS and run migrations.
  - Validate private networking and security hardening.
  - Configure HTTPS/custom domain when needed.

- **Week 4 - Stabilization & Handover**
  - Execute baseline performance and smoke tests.
  - Add monitoring, alerting, and operations runbook.
  - Review real monthly cost and optimization actions.

## 5. Budget

Monthly estimate (USD), based on current AWS pricing references and Spendwise architecture.
...
| Amazon Route 53 | Hosted zone + DNS queries | $0 - $5 |
...
- Disable custom domain (Route 53 + ACM) in early stages and use default endpoints.
